/*
  ...
  Copyright (c) 2003-2004 by Toni Wilen <twilen@arabuusimiehet.com>
      and Vincent Joguin
  Enhancements and severing from UAE (c) 2001, 2006, 2015 
      by Thomas Harte <thomas.harte@gmail.com>
...
static uint8_t *expand_tree (uint8_t *stream, NODE *node)
{
	if (temp & temp2) {
		free (node->left);
		node->left = 0;
...
static uint8_t *values_tree8 (uint8_t *stream, NODE *node)
{
	if (node->left == 0) {
		node->v = *stream++;
		return stream;
...
static uint8_t *values_tree16 (uint8_t *stream, NODE *node)
{
	if (node->left == 0) {
...
static void fdi_decode (uint8_t *stream, int size, uint8_t *out)
{
	int i;
...
/* add one byte */
static void byte_add (FDI *fdi, uint8_t v)
{
	int i;
	for (i = 7; i >= 0; i--)
		bit_add (fdi, v & (1 << i));
...
/* add one word */
static void word_add (FDI *fdi, uint16_t v)
{
	byte_add (fdi, (uint8_t)(v >> 8));
	byte_add (fdi, (uint8_t)v);
}
/* add one byte and mfm encode it */
static void byte_mfm_add (FDI *fdi, uint8_t v)
{
	int i;
	for (i = 7; i >= 0; i--)
...
static void bytes_mfm_add (FDI *fdi, uint8_t v, int len)
{
	int i;
	for (i = 0; i < len; i++) byte_mfm_add (fdi, v);
}
/* add one mfm encoded word and re-mfm encode it */
static void word_post_mfm_add (FDI *fdi, uint16_t v)
{
	int i;
	for (i = 14; i >= 0; i -= 2)
		bit_mfm_add (fdi, v & (1 << i));
...
/*static int amiga_check_track (FDI *fdi)
{
	...
	//memcpy (mbuf + fwlen, mbuf, fwlen * sizeof (uint16_t));
...
static void amiga_data_raw (FDI *fdi, uint8_t *secbuf, uint8_t *crc, int len)
{
	int i;
	uint8_t crcbuf[4];
...
static void amiga_data (FDI *fdi, uint8_t *secbuf)
{
...
static void amiga_sector_header	(FDI *fdi, uint8_t *header, uint8_t *data, int sector, int untilgap)
{
...
static uint16_t ibm_crc (uint8_t byte, int reset)
{
	static uint16_t crc;
	int i;
...
static void ibm_data (FDI *fdi, uint8_t *data, uint8_t *crc, int len)
{
	int i;
	uint8_t crcbuf[2];
	uint16_t crcv = 0;
...
static void ibm_sector_header (FDI *fdi, uint8_t *data, uint8_t *crc, int secnum, int pre)
{
	uint8_t secbuf[5];
	uint8_t crcbuf[2];
	uint16_t crcv;
...
static uint8_t *fdi_decompress (int pulses, uint8_t *sizep, uint8_t *src, int *dofree)
{
...
static void dumpstream(int track, uint8_t *stream, int len)
{
#if 0
    char name[100];
    FILE *f;
...
static inline void addbit (uint8_t *p, int bit)
{
...
static void fdi2_decode (FDI *fdi, unsigned long totalavg, uint32_t *avgp, uint32_t *minp, uint32_t *maxp, uint8_t *idx, int maxidx, int *indexoffsetp, int pulses, int mfm)
{
			...
			if (idx[i] >= maxidx) { /* stable pulse */
				...
				if (randval < (RAND_MAX / 2)) {
					if (randval > (RAND_MAX / 4)) {
						if (randval <= (3 * (uint64_t)RAND_MAX / 8))
...
static void fdi2_celltiming (FDI *fdi, unsigned long totalavg, int bitoffset, uint16_t *out)
{
	uint16_t *pt2, *pt;
	double avg_bit_len;
	int i;
...
void fdi2raw_header_free (FDI *fdi)
{
	...
	free (fdi->track_dst_buffer_timing);
...
FDI *fdi2raw_header(FILE *f)
{
	...
	fread (fdi->header, 1, 2048, fdi->file );
	...
	if (fdi->header[140] * 256 + fdi->header[141] >= 2 * 256 + 1) {
		/*uint32_t crc = get_crc32(fdi->header, 508);
		uint32_t crc2 = (fdi->header[508] << 24) | (fdi->header[509] << 16) | (fdi->header[510] << 8) | fdi->header[511];
		if (crc != crc2) {
			fdi_free(fdi);
			return NULL;
		}*/
	...
	fdi->track_dst_buffer_timing = malloc (MAX_TIMING_BUFFER);
...
static int fdi2raw_loadrevolution_2 (FDI *fdi, uint16_t *mfmbuf, uint16_t *tracktiming, int track, int *tracklength, int *indexoffsetp, int *multirev, int mfm)
{
...
int fdi2raw_loadrevolution (FDI *fdi, uint16_t *mfmbuf, uint16_t *tracktiming, int track, int *tracklength, int mfm)
{
	track ^= fdi->reversed_side;
	return fdi2raw_loadrevolution_2 (fdi, mfmbuf, tracktiming, track, tracklength, 0, 0, mfm);
}

int fdi2raw_loadtrack (FDI *fdi, uint16_t *mfmbuf, uint16_t *tracktiming, int track, int *tracklength, int *indexoffsetp, int *multirev, int mfm)
{
	uint8_t *p;
	int outlen, i;
	struct fdi_cache *cache = &fdi->cache[track];
	...
	fread (fdi->track_src_buffer, 1, fdi->track_src_len, fdi->file);
	memset (fdi->track_dst_buffer, 0, MAX_DST_BUFFER);
